...
public class AnnotationsUtils{
	...
	public Optional<String> getStringProperty(RAnnotation annotation, String propertyName) {
		return getProperty(annotation, propertyName)
			.map(rc -> {
	...
	public Optional<RConst> getProperty(RAnnotation annotation, String propertyName) {
		...
		RAnnotationDef def = getDef(annotation.getName())
			.orElseThrow(() -> new SubstemaException("Can't find annotation def for " + annotation));
	...
	public Optional<RProperty> getDefaultProperty(RAnnotationDef def) {
		PStream<RProperty> withoutDefaults =
			def.getProperties().lazy().filter(p -> p.getDefaultValue().isPresent() == false);
